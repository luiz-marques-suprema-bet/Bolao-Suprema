-- ============================================================================
-- Bolao Suprema · streak = so resultado certo (>=5) + artilheiro 100% manual
-- ----------------------------------------------------------------------------
-- Aplicado em producao via Management API (hotfix). Idempotente.
--   1) Artilheiro: limpa o award decidido por engano (gatilho automatico marcou
--      o lider apos 1 jogo) e DESLIGA o auto-decisao. O artilheiro passa a ser
--      definido SOMENTE no fim, manualmente (admin_set_tournament_scorer).
--   2) refresh_ranking_snapshots: streak passa a contar apenas jogos em que o
--      palpiteiro acertou AO MENOS o resultado (>=5 pts), nao mais qualquer
--      ponto >0 (o 1pt parcial nao mantem mais a sequencia).
-- ============================================================================

-- 1) Artilheiro volta a ser manual
update public.tournament_awards set decided_at = null, player_id = null
  where id = 'scorer' and decided_at is not null;

do $$ begin
  if exists (
    select 1 from pg_trigger
    where tgname = 'trg_player_goal_totals_sync_top_scorer' and not tgisinternal
  ) then
    execute 'alter table public.player_goal_totals disable trigger trg_player_goal_totals_sync_top_scorer';
  end if;
end $$;

-- 2) refresh_ranking_snapshots com streak = resultado certo (>=5)
